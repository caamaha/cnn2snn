...
void _run_neurongroup_1_thresholder_1_codeobject()
{	
	...
	///// CONSTANTS ///////////
	const int _numv = 2880;
const int _num_underspace = 2881;
	///// POINTERS ////////////
 	
 double* __restrict  _ptr_array_neurongroup_1_v = _array_neurongroup_1_v;
 int32_t* __restrict  _ptr_array_neurongroup_1__underspace = _array_neurongroup_1__underspace;
    ...
    long _count = 0;
    for(int _idx=0; _idx<2880; _idx++)
    {
        const int _vectorisation_idx = _idx;
                
        const double v = _ptr_array_neurongroup_1_v[_idx];
        const bool _cond = v < 0;

        if(_cond) {
            _ptr_array_neurongroup_1__underspace[_count++] = _idx;
        }
    }
    _ptr_array_neurongroup_1__underspace[2880] = _count;

    const double _run_time = (double)(std::clock() -_start_time)/CLOCKS_PER_SEC;
    neurongroup_1_thresholder_1_codeobject_profiling_info += _run_time;
}
