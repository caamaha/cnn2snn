...
{	
	...
	///// CONSTANTS ///////////
	const int _numv = 500;
const int _num_spikespace = 501;
	///// POINTERS ////////////
 	
 double* __restrict  _ptr_array_neurongroup_4_v = _array_neurongroup_4_v;
 int32_t* __restrict  _ptr_array_neurongroup_4__spikespace = _array_neurongroup_4__spikespace;
    ...
    long _count = 0;
    for(int _idx=0; _idx<500; _idx++)
    {
        const int _vectorisation_idx = _idx;
                
        const double v = _ptr_array_neurongroup_4_v[_idx];
        const bool _cond = v >= 1;

        if(_cond) {
            _ptr_array_neurongroup_4__spikespace[_count++] = _idx;
        }
    }
    _ptr_array_neurongroup_4__spikespace[500] = _count;

    const double _run_time = (double)(std::clock() -_start_time)/CLOCKS_PER_SEC;
    neurongroup_4_thresholder_codeobject_profiling_info += _run_time;
}
