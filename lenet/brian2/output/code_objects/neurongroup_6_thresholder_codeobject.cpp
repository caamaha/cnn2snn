...
const int _num_spikespace = 11;
 ...
 int32_t* __restrict  _ptr_array_neurongroup_6__spikespace = _array_neurongroup_6__spikespace;
    ...
    for(int _idx=0; _idx<10; _idx++)
    {
        ...
        const double v = _ptr_array_neurongroup_6_v[_idx];
        const bool _cond = v >= 2;

        if(_cond) {
            _ptr_array_neurongroup_6__spikespace[_count++] = _idx;
        }
    }
    _ptr_array_neurongroup_6__spikespace[10] = _count;

    const double _run_time = (double)(std::clock() -_start_time)/CLOCKS_PER_SEC;
    neurongroup_6_thresholder_codeobject_profiling_info += _run_time;
}
