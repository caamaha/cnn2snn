...
def extract_data(filenames):
    ...
    for f in filenames:
        ...
        # 读取数据
        buf = bytestream.read(
            TRAIN_NUM * (IMAGE_SIZE * IMAGE_SIZE * NUM_CHANNELS + LABEL_SIZE))
        # 把数据流转化为np的数组
        data = np.frombuffer(buf, dtype=np.uint8)
        # 改变数据格式
        data = data.reshape(
            TRAIN_NUM, LABEL_SIZE + IMAGE_SIZE * IMAGE_SIZE * NUM_CHANNELS)
        # 分割数组
        labels_images = np.hsplit(data, [LABEL_SIZE])

        label = labels_images[0].reshape(TRAIN_NUM)
        image = labels_images[1].reshape(
            TRAIN_NUM, NUM_CHANNELS, IMAGE_SIZE, IMAGE_SIZE)
