        ...
        for h in range(in_ind.sz2):
            for w in range(in_ind.sz1):
                for d in range(in_ind.sz0):
                    i = in_ind.ind3(h, w, d)
                    if in_ind.sz1 > 1:
                        pre_ind.append(d * in_ind.sz1 * in_ind.sz2 + h * in_ind.sz1 + w)
                    else:
                        pre_ind.append(i)
                    post_ind.append(n)
...
poo2_ip1_ind        = dim3_ind(pool2_output_size, pool2_output_size, pool2_kernel_num)
...
#------------------------------------------------------------------------------
pretrained_lenet = sio.loadmat('../tensorflow/output/weights/lenet_avg_pooling.mat')
...
post_ind = []
ip_w     = []
create_ip_connections(poo2_ip1_ind, ip1_output_n, ip1_weights, pre_ind, post_ind, ip_w)
...
conv2_mon = SpikeMonitor(pool2_group)
